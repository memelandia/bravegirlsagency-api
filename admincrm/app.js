...
(function () {
  ...
  function navigate(route) {
    ...
    const titles = {
      ...
      trazabilidad:['Trazabilidad de Pagos', 'Investigación retroactiva: quién cobró cuánto en qué mes y por dónde'],
    ...
    if (route === 'trazabilidad')return renderTrazabilidad(view);
...
<div style="font-family:Arial,Helvetica,sans-serif;padding:20px 24px;color:${VALUE};background:#fff;width:100%;box-sizing:border-box">
  ...
  // ═══════════════════════════════════════════════════════
  // TRAZABILIDAD · matriz Excel para investigar pagos retroactivos
  //   Completamente aislada del resto del dashboard.
  //   Tabs por año (2023-actual). Filas chatters × 12 meses.
  //   Estados: pendiente | encontrado | facturado.
  // ═══════════════════════════════════════════════════════
  let trazState = { año: new Date().getFullYear() };

  async function renderTrazabilidad(view) {
    view.innerHTML = Skel.table(8);
    try {
      const r = await api('traz-grid', { params: { año: trazState.año } });
      const chatters = r.chatters || [];
      const pagos = r.pagos_by_chatter || {};
      const k = r.kpis || {};

      // Calcular años disponibles (2023 → current+1)
      const currentYear = new Date().getFullYear();
      const years = [];
      for (let y = 2023; y <= currentYear + 1; y++) years.push(y);

      const yearTabsHtml = years.map(y =>
        `<button class="traz-year-tab ${y === trazState.año ? 'active' : ''}" data-year="${y}">${y}</button>`
      ).join('');

      const monthHeaders = MESES_SHORT.map((m, i) =>
        `<th class="traz-month-h" data-mes="${String(i+1).padStart(2,'0')}">${m.charAt(0).toUpperCase() + m.slice(1)}</th>`
      ).join('');

      const fmtCellMonto = (m) => {
        if (m == null) return '';
        const n = Number(m);
      ...
      const renderCell = (chatterId, mes) => {
        const pago = (pagos[String(chatterId)] || {})[mes];
        if (!pago) {
          return `<td class="traz-cell estado-vacio" data-chatter-id="${chatterId}" data-mes="${mes}" title="Click para registrar">·</td>`;
        }
        const estado = pago.estado || 'pendiente';
        const tooltip = [
          pago.medio_pago ? `Vía: ${pago.medio_pago}` : null,
          pago.tx_id ? `TX: ${pago.tx_id}` : null,
          pago.fecha_envio ? `Fecha: ${pago.fecha_envio}` : null,
          pago.notas ? `Notas: ${pago.notas}` : null
        ].filter(Boolean).join(' · ');
        const monto = pago.monto != null ? fmtCellMonto(pago.monto) : '';
        return `<td class="traz-cell estado-${estado}" data-chatter-id="${chatterId}" data-mes="${mes}" title="${escapeHtml(tooltip || 'Click para editar')}">
          <span class="traz-cell-monto">${monto}</span>
        </td>`;
      };

      const rowsHtml = chatters.length === 0
        ? `<tr><td colspan="14" class="empty-state center" style="padding:40px"><div class="big">🔍</div><div>Sin chatters cargados.</div><div style="margin-top:10px"><button class="btn-primary" id="traz-empty-add" style="width:auto;display:inline-flex">+ Agregar el primero</button></div></td></tr>`
        : chatters.map(c => {
            const cells = [];
            for (let m = 1; m <= 12; m++) {
              const mes = `${trazState.año}-${String(m).padStart(2, '0')}`;
              cells.push(renderCell(c.id, mes));
            }
            const notasIcon = c.notas ? `<span title="${escapeHtml(c.notas)}" style="color:var(--info);margin-left:4px">📝</span>` : '';
            return `
              <tr>
                <td class="traz-name-cell">
                  <button class="traz-name-btn" data-chatter-id="${c.id}" title="Editar chatter">${escapeHtml(c.nombre)}</button>
                  ${notasIcon}
                </td>
                ${cells.join('')}
                <td class="traz-actions">
                  <button class="btn-danger btn-row-del" data-traz-del="${c.id}" title="Borrar chatter (cascada borra sus pagos)">✕</button>
                </td>
              </tr>
            `;
          }).join('');
      ...
      view.innerHTML = `
        <div class="card" style="margin-bottom:16px">
          <div class="traz-toolbar">
            <div class="traz-year-tabs">${yearTabsHtml}</div>
            <div class="traz-actions-bar">
              <button class="btn-secondary" id="traz-export" style="width:auto;padding:8px 14px;font-size:0.82rem">📥 Exportar CSV</button>
              <button class="btn-primary" id="traz-add" style="width:auto;padding:8px 14px;font-size:0.82rem">+ Agregar chatter</button>
            </div>
          </div>

          <div class="traz-kpi-row" style="margin-top:14px">
            <div class="traz-kpi">
              <div class="traz-kpi-lbl">Chatters</div>
              <div class="traz-kpi-val">${k.chatters_count || 0}</div>
            </div>
            <div class="traz-kpi traz-kpi-pending">
              <div class="traz-kpi-lbl">Pendientes</div>
              <div class="traz-kpi-val">${k.pendientes || 0}</div>
            </div>
            <div class="traz-kpi traz-kpi-found">
              <div class="traz-kpi-lbl">Encontrados</div>
              <div class="traz-kpi-val">${k.encontrados || 0}</div>
            </div>
            <div class="traz-kpi traz-kpi-invoiced">
              <div class="traz-kpi-lbl">Facturados</div>
              <div class="traz-kpi-val">${k.facturados || 0}</div>
            </div>
          </div>
        </div>

        <div class="traz-matrix-wrap">
          <table class="traz-matrix">
            <thead>
              <tr>
                <th class="traz-name-h">Chatter</th>
                ${monthHeaders}
                <th class="traz-actions-h"></th>
              </tr>
            </thead>
            <tbody>${rowsHtml}</tbody>
          </table>
        ...
        <p class="muted" style="font-size:0.74rem;margin-top:14px;text-align:center">
          <strong>Convención de colores:</strong>
          <span style="color:#9CA3AF;margin:0 6px">⚪ pendiente</span>
          <span style="color:#FB923C;margin:0 6px">🟠 encontrado</span>
          <span style="color:#22C55E;margin:0 6px">🟢 facturado</span>
          · Esta sección NO afecta P&amp;L, Resumen ni Liquidación.
        </p>
      `;

      // Listeners
      view.querySelectorAll('.traz-year-tab').forEach(tab => {
        tab.addEventListener('click', () => {
          trazState.año = Number(tab.dataset.year);
          renderTrazabilidad(view);
        });
      });
      document.getElementById('traz-add')?.addEventListener('click', () => openTrazAddChatterModal(null, view));
      document.getElementById('traz-empty-add')?.addEventListener('click', () => openTrazAddChatterModal(null, view));
      document.getElementById('traz-export')?.addEventListener('click', () => {
        const url = `${API}?action=traz-export&año=${trazState.año}`;
        // Forzar descarga con token via fetch
        fetch(url, { headers: { 'X-Admin-Token': token } })
          .then(r => r.blob())
          .then(blob => {
            const a = document.createElement('a');
            a.href = URL.createObjectURL(blob);
            a.download = `trazabilidad_${trazState.año}.csv`;
            a.click();
      ...
      view.querySelectorAll('.traz-name-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const c = chatters.find(x => x.id === Number(btn.dataset.chatterId));
          if (c) openTrazAddChatterModal(c, view);
        });
      });
      view.querySelectorAll('[data-traz-del]').forEach(btn => {
        btn.addEventListener('click', async () => {
          const id = Number(btn.dataset.trazDel);
          const c = chatters.find(x => x.id === id);
          if (!confirm(`¿Borrar a "${c?.nombre || 'este chatter'}" y todos sus registros de pago? Esta acción no se puede deshacer.`)) return;
          try {
            await api('traz-chatter-delete', { method: 'POST', params: { id } });
            toast('Borrado', 'success');
            renderTrazabilidad(view);
          } catch (e) { toast('Error: ' + e.message, 'error'); }
        });
      });
      view.querySelectorAll('.traz-cell').forEach(cell => {
        cell.addEventListener('click', () => {
          const chatterId = Number(cell.dataset.chatterId);
          const mes = cell.dataset.mes;
          const c = chatters.find(x => x.id === chatterId);
          const pago = (pagos[String(chatterId)] || {})[mes] || null;
          if (c) openTrazPagoModal(c, mes, pago, view);
        });
      });
    } catch (e) {
      view.innerHTML = `<div class="card center muted">⚠️ ${e.message}</div>`;
  ...
  function openTrazAddChatterModal(existing, viewRef) {
    ...
    backdrop.innerHTML = `
      <div class="modal" style="max-width:480px">
        <div class="modal-header">
          <div class="modal-title">${isEdit ? '✎ Editar' : '+ Agregar'} chatter en trazabilidad</div>
          <button class="modal-close" type="button">✕</button>
        </div>
        <form id="traz-c-form">
          <div class="form-grid" style="grid-template-columns:1fr;gap:12px">
            <div>
              <label for="traz-c-nombre">Nombre *</label>
              <input type="text" id="traz-c-nombre" required value="${escapeHtml(existing?.nombre || '')}" placeholder="ej: Brian, JM, Sofi, Alfonso...">
            </div>
            <div>
              <label for="traz-c-notas">Notas (opcional)</label>
              <textarea id="traz-c-notas" rows="3" placeholder="Contexto: cuándo trabajó, dónde está hoy, etc.">${escapeHtml(existing?.notas || '')}</textarea>
            </div>
            <div>
              <label for="traz-c-orden">Orden de aparición</label>
              <input type="number" id="traz-c-orden" step="1" value="${existing?.orden || 0}">
            </div>
          </div>
          <div class="form-actions" style="margin-top:14px">
            <button type="button" class="btn-secondary modal-cancel">Cancelar</button>
    ...
    backdrop.querySelector('.modal-cancel').addEventListener('click', close);
    backdrop.addEventListener('click', e => { if (e.target === backdrop) close(); });
    setTimeout(() => backdrop.querySelector('#traz-c-nombre').focus(), 50);

    backdrop.querySelector('#traz-c-form').addEventListener('submit', async (e) => {
      e.preventDefault();
      const body = {
        id: existing?.id || undefined,
        nombre: backdrop.querySelector('#traz-c-nombre').value.trim(),
        notas: backdrop.querySelector('#traz-c-notas').value.trim() || null,
        orden: Number(backdrop.querySelector('#traz-c-orden').value || 0)
      };
      if (!body.nombre) { toast('Nombre requerido', 'error'); return; }
      try {
        await api('traz-chatter', { method: 'POST', body });
        toast(isEdit ? 'Guardado' : 'Chatter agregado', 'success');
        close();
        if (viewRef) renderTrazabilidad(viewRef);
      } catch (e) { toast('Error: ' + e.message, 'error'); }
    });
  }

  function openTrazPagoModal(chatter, mes, existing, viewRef) {
    const pago = existing || {};
    const estado = pago.estado || 'pendiente';
    const medioOpts = MEDIOS_PAGO.map(m => `<option value="${m}">`).join('');
    ...
    backdrop.innerHTML = `
      <div class="modal" style="max-width:540px">
        <div class="modal-header">
          <div class="modal-title">${escapeHtml(chatter.nombre)} · ${fmtMesNice(mes)}</div>
          <button class="modal-close" type="button">✕</button>
        </div>
        <form id="traz-p-form">
          <div style="margin-bottom:14px">
            <label>Estado</label>
            <div class="traz-estado-radios">
              <label class="traz-estado-radio ${estado === 'pendiente' ? 'active' : ''}">
                <input type="radio" name="estado" value="pendiente" ${estado === 'pendiente' ? 'checked' : ''}>
                <span>❓ Pendiente</span>
              </label>
              <label class="traz-estado-radio ${estado === 'encontrado' ? 'active' : ''}">
                <input type="radio" name="estado" value="encontrado" ${estado === 'encontrado' ? 'checked' : ''}>
                <span>✅ Encontrado</span>
              </label>
              <label class="traz-estado-radio ${estado === 'facturado' ? 'active' : ''}">
                <input type="radio" name="estado" value="facturado" ${estado === 'facturado' ? 'checked' : ''}>
                <span>📄 Facturado</span>
              </label>
            </div>
          </div>
          ...
          <div class="form-grid" style="grid-template-columns:1fr 1fr;gap:12px">
            <div>
              <label for="traz-p-monto">Monto</label>
              <input type="number" id="traz-p-monto" step="0.01" value="${pago.monto || ''}" placeholder="0.00">
            </div>
            <div>
              <label for="traz-p-moneda">Moneda</label>
              <select id="traz-p-moneda">
                <option value="USD" ${(pago.moneda || 'USD') === 'USD' ? 'selected' : ''}>USD</option>
                <option value="EUR" ${pago.moneda === 'EUR' ? 'selected' : ''}>EUR</option>
                <option value="ARS" ${pago.moneda === 'ARS' ? 'selected' : ''}>ARS</option>
              </select>
            </div>
            <div class="full">
              <label for="traz-p-medio">Medio de pago</label>
              <input type="text" id="traz-p-medio" list="dl-medios-traz" value="${escapeHtml(pago.medio_pago || '')}" placeholder="Binance USDT (TRC20), Wise, Skrill...">
              <datalist id="dl-medios-traz">${medioOpts}</datalist>
            </div>
            <div>
              <label for="traz-p-tx">TX ID / Referencia</label>
              <input type="text" id="traz-p-tx" value="${escapeHtml(pago.tx_id || '')}" placeholder="Hash de transacción">
            </div>
            <div>
              <label for="traz-p-fecha">Fecha de envío</label>
              <input type="date" id="traz-p-fecha" value="${pago.fecha_envio || ''}">
            </div>
            <div class="full">
              <label for="traz-p-notas">Notas</label>
              <textarea id="traz-p-notas" rows="2" placeholder="Cualquier detalle adicional">${escapeHtml(pago.notas || '')}</textarea>
            </div>
          </div>

          <div class="form-actions" style="margin-top:14px;justify-content:space-between">
            <button type="button" class="btn-danger" id="traz-p-clear" style="width:auto;padding:8px 14px">🗑 Vaciar celda</button>
            <div style="display:flex;gap:8px">
              <button type="button" class="btn-secondary modal-cancel">Cancelar</button>
              <button type="submit" class="btn-primary">💾 Guardar</button>
            </div>
    ...
    // Activar radio visual
    backdrop.querySelectorAll('.traz-estado-radio input').forEach(r => {
      r.addEventListener('change', () => {
        backdrop.querySelectorAll('.traz-estado-radio').forEach(l => l.classList.remove('active'));
        r.closest('.traz-estado-radio').classList.add('active');
      });
    });

    backdrop.querySelector('#traz-p-clear').addEventListener('click', async () => {
      if (!confirm('¿Vaciar esta celda? Estado vuelve a pendiente y se borran monto/medio/tx_id.')) return;
      try {
        await api('traz-pago', {
          method: 'POST',
          body: {
            trazabilidad_chatter_id: chatter.id, mes,
            estado: 'pendiente', monto: null, moneda: 'USD',
            medio_pago: null, tx_id: null, fecha_envio: null, notas: null
          }
        });
        toast('Vaciado', 'success');
        close();
        if (viewRef) renderTrazabilidad(viewRef);
      } catch (e) { toast('Error: ' + e.message, 'error'); }
    });

    backdrop.querySelector('#traz-p-form').addEventListener('submit', async (e) => {
      e.preventDefault();
      const body = {
        trazabilidad_chatter_id: chatter.id,
        mes,
        estado: backdrop.querySelector('input[name="estado"]:checked').value,
        monto: backdrop.querySelector('#traz-p-monto').value || null,
        moneda: backdrop.querySelector('#traz-p-moneda').value,
        medio_pago: backdrop.querySelector('#traz-p-medio').value.trim() || null,
        tx_id: backdrop.querySelector('#traz-p-tx').value.trim() || null,
        fecha_envio: backdrop.querySelector('#traz-p-fecha').value || null,
        notas: backdrop.querySelector('#traz-p-notas').value.trim() || null
      };
      try {
        await api('traz-pago', { method: 'POST', body });
