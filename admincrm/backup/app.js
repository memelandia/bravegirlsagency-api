...
(function () {
  ...
  async function renderResumen(view) {
    ...
    try {
      ...
      const k = r.kpis;
      const STAT = (color, ico, label, value, sub) => `
        <div class="stat-card">
          <div class="stat-card-ico" style="background:linear-gradient(135deg,${color.bg1},${color.bg2});color:${color.fg}">${ico}</div>
          <div class="stat-card-label">${label}</div>
          <div class="stat-card-value">${value}</div>
          <div class="stat-card-delta muted">${sub || ''}</div>
        </div>`;
      const C = {
        pink:   { bg1:'rgba(255,31,142,.2)',  bg2:'rgba(255,31,142,.05)',  fg:'var(--pink)' },
        purple: { bg1:'rgba(168,85,247,.2)',  bg2:'rgba(168,85,247,.05)',  fg:'var(--purple)' },
        blue:   { bg1:'rgba(59,130,246,.2)',  bg2:'rgba(59,130,246,.05)',  fg:'var(--blue)' },
        gold:   { bg1:'rgba(255,215,0,.2)',   bg2:'rgba(255,215,0,.05)',   fg:'var(--gold)' },
        orange: { bg1:'rgba(251,146,60,.2)',  bg2:'rgba(251,146,60,.05)',  fg:'var(--orange)' },
        green:  { bg1:'rgba(16,185,129,.2)',  bg2:'rgba(16,185,129,.05)',  fg:'var(--green)' }
      };
      ...
      const gastosRows = (r.gastos || []).slice(0, 8).map(g => `
        <tr>
          <td>${g.descripcion || '—'}</td>
      ...
      view.innerHTML = `
        <div class="stats-grid">
          ${STAT(C.pink,   '$', 'Ingreso bruto',      fmtMoney(k.total_a_cobrar), `${k.cuentas_con_cierre} cuentas cerradas · facturación total a modelos`)}
          ${STAT(C.purple, '👥','Suscripciones',      fmtMoney(k.suscripciones), 'no comisionables')}
          ${STAT(C.gold,   '↗', 'Ganancia bruta agencia', fmtMoney(k.ganancia_bruta), '% del plan sobre fact s/subs')}
          ${STAT(C.orange, '↘', 'Gastos totales',     fmtMoney(k.gastos_fijos), desglose)}
          ${STAT(C.green,  '★', 'NETO OWNER',         fmtMoney(k.neto_owner), 'ingreso − chatters − sup − equipo − OM − gastos')}
          ${STAT(C.blue,   '▦', 'Facturado',          fmtMoney(k.fact_total), `${k.facturas_mes} facturas emitidas`)}
        </div>

        <div style="display:grid;grid-template-columns:2fr 1fr;gap:18px;margin-top:24px">
          <div class="table-wrap">
            <div class="table-toolbar">
              <h3>💎 Cobros a Modelos <span class="count">${(r.cobros || []).length}</span></h3>
              <button class="btn-ghost-small" onclick="location.hash='facturacion'">Ver detalle →</button>
            </div>
            <table>
              <thead>
                <tr><th>Modelo</th><th>Total a cobrar</th><th>Pagado</th><th>Pendiente</th><th>Estado</th></tr>
              </thead>
              <tbody>${cobrosRows || '<tr><td colspan="5" class="empty-state">Aún no cargaste cierres este mes.</td></tr>'}</tbody>
            </table>
          </div>

          <div class="table-wrap">
            <div class="table-toolbar">
              <h3>▼ Gastos del mes <span class="count">${(r.gastos || []).length}</span></h3>
              <button class="btn-ghost-small" onclick="location.hash='gastos'">Gestionar →</button>
            </div>
            <table>
              <thead><tr><th>Descripción</th><th>Cat.</th><th style="text-align:right">Monto</th></tr></thead>
              <tbody>${gastosRows || '<tr><td colspan="3" class="empty-state">Sin gastos registrados.</td></tr>'}</tbody>
            </table>
          </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-top:18px">
          <div class="card center" style="padding:18px">
            <div class="muted" style="font-size:.72rem;text-transform:uppercase;letter-spacing:.1em;font-weight:700">Modelos activos</div>
            <div style="font-family:var(--font-display);font-size:2rem;font-weight:700;margin-top:4px">${k.modelos_activos}</div>
          </div>
          <div class="card center" style="padding:18px">
            <div class="muted" style="font-size:.72rem;text-transform:uppercase;letter-spacing:.1em;font-weight:700">Chatters activos</div>
            <div style="font-family:var(--font-display);font-size:2rem;font-weight:700;margin-top:4px">${k.chatters_activos}</div>
          </div>
          <div class="card center" style="padding:18px">
            <div class="muted" style="font-size:.72rem;text-transform:uppercase;letter-spacing:.1em;font-weight:700">Facturas del mes</div>
            <div style="font-family:var(--font-display);font-size:2rem;font-weight:700;margin-top:4px">${k.facturas_mes}</div>
          </div>
        </div>
      `;
    } catch (e) {
  ...
  // ───────── Vista: Ajustes (mínima) ─────────
  async function renderAjustes(view) {
    view.innerHTML = `
      <div class="card">
        <div class="card-title">⚙ Ajustes globales</div>
        <p class="card-sub">Configuración general del sistema.</p>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:18px">
          <div>
            <label>Transaction fee del mes seleccionado (%)</label>
            <input type="number" step="0.01" id="txfee" placeholder="3, 4 o 5" disabled>
            <p style="font-size:0.75rem;color:var(--text-mute);margin-top:6px">Disponible próximamente, cuando avancemos a Fase 2.</p>
          </div>
          <div>
            <label>Versión del sistema</label>
            <input type="text" value="Admin Fase 1 — Catálogo" disabled>
          </div>
        </div>
      </div>
      ...
      <div class="card" style="margin-top:18px">
        ...
        <p class="card-sub">Tu token está guardado en sessionStorage. Cerrá sesión cuando termines.</p>
        <button class="btn-secondary" id="logout-also" style="margin-top:14px">Cerrar sesión</button>
      </div>
    `;
    ...
    try {
      ...
      try {
        await html2pdf().set({
          ...
          html2canvas: { scale: 2, backgroundColor: '#fce7f3', useCORS: true, logging: false },
      ...
      view.querySelectorAll('[data-reprint]').forEach(b => {
        b.addEventListener('click', async () => {
          ...
          try {
            ...
            const wrapper = document.createElement('div');
            wrapper.style.cssText = 'position:fixed;top:-9999px;left:0;width:800px';
            wrapper.innerHTML = html;
            document.body.appendChild(wrapper);
            await html2pdf().set({
              margin: 0,
