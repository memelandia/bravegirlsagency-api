...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'resumen-mes') {
      ...
      // Convierte EUR→USD multiplicando por cotizacion_eur cuando moneda='EUR'
      // (fact_total en Carmen se registra en EUR; el resto queda tal cual).
      const tot = await sql`
        SELECT
          COALESCE(SUM(fact_total * CASE WHEN moneda='EUR' THEN COALESCE(NULLIF(cotizacion_eur,0),1) ELSE 1 END), 0)         AS fact_total,
          COALESCE(SUM(suscripciones * CASE WHEN moneda='EUR' THEN COALESCE(NULLIF(cotizacion_eur,0),1) ELSE 1 END), 0)      AS suscripciones,
          COALESCE(SUM((fact_total - suscripciones - masivos) * CASE WHEN moneda='EUR' THEN COALESCE(NULLIF(cotizacion_eur,0),1) ELSE 1 END), 0) AS fact_sin_subs,
          COALESCE(SUM((fact_total - suscripciones - masivos) * porcentaje_aplicado / 100 * CASE WHEN moneda='EUR' THEN COALESCE(NULLIF(cotizacion_eur,0),1) ELSE 1 END), 0) AS ganancia_bruta,
          COALESCE(SUM(total_a_cobrar * CASE WHEN moneda='EUR' THEN COALESCE(NULLIF(cotizacion_eur,0),1) ELSE 1 END), 0)     AS total_a_cobrar,
          COALESCE(SUM(pago_recibido * CASE WHEN moneda='EUR' THEN COALESCE(NULLIF(cotizacion_eur,0),1) ELSE 1 END), 0)      AS pago_recibido,
          COALESCE(SUM((total_a_cobrar - pago_recibido) * CASE WHEN moneda='EUR' THEN COALESCE(NULLIF(cotizacion_eur,0),1) ELSE 1 END), 0) AS pago_pendiente,
      ...
      const histFactRes = await sql`
        SELECT mes,
               COALESCE(SUM(fact_total * CASE WHEN moneda='EUR' THEN COALESCE(NULLIF(cotizacion_eur,0),1) ELSE 1 END), 0) AS total
        FROM cierre_cuenta_mes
        GROUP BY mes
      `;
    ...
    if (action === 'pnl') {
      ...
      // Convierte EUR→USD via cotizacion_eur para totales agregados en P&L.
      const cierres = await sql`
        SELECT
          ...
          COALESCE(SUM((fact_total - suscripciones - masivos) * CASE WHEN moneda='EUR' THEN COALESCE(NULLIF(cotizacion_eur,0),1) ELSE 1 END), 0) AS fact_sin_subs,
          COALESCE(SUM((fact_total - suscripciones - masivos) * porcentaje_aplicado / 100 * CASE WHEN moneda='EUR' THEN COALESCE(NULLIF(cotizacion_eur,0),1) ELSE 1 END), 0) AS ganancia_bruta,
          COALESCE(SUM(total_a_cobrar * CASE WHEN moneda='EUR' THEN COALESCE(NULLIF(cotizacion_eur,0),1) ELSE 1 END), 0) AS ingreso_bruto
