-- CREAR TODAS LAS TABLAS Y DATOS REALES
-- Ejecutar TODO en Neon SQL Editor

-- ============================================
-- CREAR TABLAS SI NO EXISTEN
-- ============================================

CREATE TABLE IF NOT EXISTS crm_models (
    id SERIAL PRIMARY KEY,
    handle VARCHAR(255) NOT NULL UNIQUE,
    estimado_facturacion_mensual INTEGER DEFAULT 0,
    prioridad INTEGER DEFAULT 3 CHECK (prioridad >= 1 AND prioridad <= 5),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_crm_models_prioridad ON crm_models(prioridad DESC);
CREATE INDEX IF NOT EXISTS idx_crm_models_handle ON crm_models(handle);

CREATE TABLE IF NOT EXISTS crm_chatters (
    id SERIAL PRIMARY KEY,
    nombre VARCHAR(255) NOT NULL,
    estado VARCHAR(50) DEFAULT 'activo' CHECK (estado IN ('activo', 'prueba', 'pausa')),
    nivel VARCHAR(50) DEFAULT 'junior' CHECK (nivel IN ('junior', 'mid', 'senior')),
    pais VARCHAR(100),
    disponibilidad JSONB DEFAULT '{}',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_crm_chatters_estado ON crm_chatters(estado);
CREATE INDEX IF NOT EXISTS idx_crm_chatters_nivel ON crm_chatters(nivel);

CREATE TABLE IF NOT EXISTS crm_assignments (
    id SERIAL PRIMARY KEY,
    chatter_id INTEGER NOT NULL REFERENCES crm_chatters(id) ON DELETE CASCADE,
    model_id INTEGER NOT NULL REFERENCES crm_models(id) ON DELETE CASCADE,
    horario JSONB DEFAULT '{}',
    estado VARCHAR(50) DEFAULT 'activa' CHECK (estado IN ('activa', 'prueba', 'reemplazo')),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_crm_assignments_chatter ON crm_assignments(chatter_id);
CREATE INDEX IF NOT EXISTS idx_crm_assignments_model ON crm_assignments(model_id);
CREATE INDEX IF NOT EXISTS idx_crm_assignments_estado ON crm_assignments(estado);

CREATE TABLE IF NOT EXISTS crm_social_accounts (
    id SERIAL PRIMARY KEY,
    model_id INTEGER NOT NULL REFERENCES crm_models(id) ON DELETE CASCADE,
    plataforma VARCHAR(50) NOT NULL CHECK (plataforma IN ('Instagram', 'TikTok', 'Telegram')),
    handle VARCHAR(255) NOT NULL,
    idioma VARCHAR(50),
    nicho VARCHAR(100),
    verticales JSONB DEFAULT '[]',
    estado VARCHAR(50) DEFAULT 'activa' CHECK (estado IN ('activa', 'warming', 'shadowban', 'pausada')),
    link_principal TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_crm_social_accounts_model ON crm_social_accounts(model_id);
CREATE INDEX IF NOT EXISTS idx_crm_social_accounts_plataforma ON crm_social_accounts(plataforma);
CREATE INDEX IF NOT EXISTS idx_crm_social_accounts_estado ON crm_social_accounts(estado);

CREATE TABLE IF NOT EXISTS crm_supervisors (
    id SERIAL PRIMARY KEY,
    nombre VARCHAR(255) NOT NULL,
    scope JSONB DEFAULT '{"type": "todos"}',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_crm_supervisors_nombre ON crm_supervisors(nombre);

CREATE TABLE IF NOT EXISTS crm_staff (
    id SERIAL PRIMARY KEY,
    nombre VARCHAR(255) NOT NULL,
    rol VARCHAR(50) NOT NULL CHECK (rol IN ('VA_EDITOR', 'AM_UPLOAD', 'CD')),
    estado VARCHAR(50) DEFAULT 'activo' CHECK (estado IN ('activo', 'prueba', 'pausado')),
    modelos_asignados JSONB DEFAULT '[]',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_crm_staff_rol ON crm_staff(rol);
CREATE INDEX IF NOT EXISTS idx_crm_staff_estado ON crm_staff(estado);

-- ============================================
-- LIMPIAR DATOS EXISTENTES
-- ============================================

DELETE FROM crm_assignments;
DELETE FROM crm_social_accounts;
DELETE FROM crm_models;
DELETE FROM crm_chatters;
DELETE FROM crm_supervisors;
DELETE FROM crm_staff;

-- ============================================
-- INSERTAR DATOS REALES
-- ============================================

INSERT INTO crm_models (handle, estimado_facturacion_mensual, prioridad) VALUES
('carmen', 15000, 5),
('bellarey', 12000, 5),
('vicky', 10000, 4),
('lexiflix', 8000, 4),
('lucygarcia', 7000, 3),
('ambar', 6000, 3),
('kate', 5000, 3);

INSERT INTO crm_chatters (nombre, estado, nivel, pais, disponibilidad) VALUES
('Nico', 'activo', 'senior', 'Argentina', '{"L": ["09:00-17:00"], "M": ["09:00-17:00"], "X": ["09:00-17:00"], "J": ["09:00-17:00"], "V": ["09:00-17:00"]}'),
('Alfonso', 'activo', 'senior', 'Argentina', '{"L": ["09:00-17:00"], "M": ["09:00-17:00"], "X": ["09:00-17:00"], "J": ["09:00-17:00"], "V": ["09:00-17:00"]}'),
('Yaye', 'activo', 'senior', 'México', '{"L": ["09:00-17:00"], "M": ["09:00-17:00"], "X": ["09:00-17:00"], "J": ["09:00-17:00"], "V": ["09:00-17:00"]}'),
('Diego', 'activo', 'mid', 'Colombia', '{"L": ["14:00-22:00"], "M": ["14:00-22:00"], "X": ["14:00-22:00"], "J": ["14:00-22:00"], "V": ["14:00-22:00"]}'),
('Kari', 'activo', 'mid', 'Venezuela', '{"L": ["10:00-18:00"], "M": ["10:00-18:00"], "X": ["10:00-18:00"], "J": ["10:00-18:00"], "V": ["10:00-18:00"]}'),
('Ana', 'prueba', 'junior', 'Colombia', '{"L": ["10:00-18:00"], "X": ["10:00-18:00"], "V": ["10:00-18:00"]}'),
('Emeley', 'prueba', 'junior', 'Perú', '{"L": ["10:00-18:00"], "X": ["10:00-18:00"], "V": ["10:00-18:00"]}'),
('Gabriela', 'prueba', 'junior', 'México', '{"L": ["10:00-18:00"], "X": ["10:00-18:00"], "V": ["10:00-18:00"]}');

INSERT INTO crm_supervisors (nombre, scope) VALUES
('Jonatan', '{"type": "todos"}');

INSERT INTO crm_staff (nombre, rol, estado, modelos_asignados) VALUES
('Franco', 'CD', 'activo', '[1, 2, 3, 4, 5, 6, 7]'),
('Jonatan', 'VA_EDITOR', 'activo', '[1, 2, 3, 4, 5, 6, 7]'),
('Aldi', 'AM_UPLOAD', 'activo', '[1, 2, 3, 4, 5, 6, 7]'),
('Josue', 'VA_EDITOR', 'activo', '[1, 2, 3, 4, 5, 6, 7]');

-- Verificar
SELECT 'MODELOS' as tabla, COUNT(*) as total FROM crm_models
UNION ALL SELECT 'CHATTERS', COUNT(*) FROM crm_chatters
UNION ALL SELECT 'SUPERVISORES', COUNT(*) FROM crm_supervisors
UNION ALL SELECT 'STAFF', COUNT(*) FROM crm_staff;
